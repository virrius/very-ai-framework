...
merge в main                  Build changed & deploy dev: собрать ТОЛЬКО изменённые образы → деплой на dev
        ...
        ▼
release tag v*                полный прогон тестов → промоут готовых образов (:latest→:vX/:stable по digest) → deploy prod
```
...
| `.github/workflows/release.yml` | tag `v*` → тесты → промоут образов (digest→`:vX`/`:stable`, без сборки) → deploy prod |
