...
push в feature/*              CI НЕ запускается — гейт до PR держит локальный pre-commit
        │ открыть PR
        ▼
PR в main                     PR CI: checks (ruff + security) ‖ tests (затронутые окружения, -m "not heavy")
        │                     по запросу: «@codex review» / «@codex answer …» / «@claude …»
        │                     (авто-ревью Codex при открытии PR — по умолчанию выкл; вкл variable CODEX_AUTO_REVIEW=true)
        │ зелёный CI + аппрув человека
        ▼
merge в main                  Full tests (main): checks + ПОЛНЫЙ прогон тестов (вкл heavy)
        │                     push-main: собрать ТОЛЬКО изменённые образы → деплой на dev
        ▼
...
| Путь | Назначение |
|---|---|
| `.github/workflows/pr.yml` | PR CI: `checks` + `tests` (затронутые окружения, без heavy); авто-ревью Codex — по умолчанию выкл (variable `CODEX_AUTO_REVIEW`) |
| `.github/workflows/main.yml` | `Full tests (main)` на push в `main` (и по кнопке): `checks` + полный прогон тестов вкл heavy |
...
| `.github/workflows/manual.yml` | ручной build и/или deploy матрицей (`workflow_dispatch`); deploy-only по тегу (`build=off`) |
| `.github/scripts/codex_review.py` | Codex-ревьюер (JSON-находки → inline-review) |
| `.github/scripts/codex_answer.py` | Codex-ответчик на `@codex answer` (PR-тред / inline-тред) |
| `.github/scripts/services.sh` | динамическое обнаружение сервисов (`services/*`, `list` / `changed` / `select`) |
| `.github/scripts/discover-envs.sh` | обнаружение тест-окружений по `pyproject` |
| `.github/scripts/affected-base.sh`, `affected-envs.sh` | вычисление затронутых окружений для PR (`affected`) |
