...
- Адрес(а) сервера(ов): один на все стенды или разные хосты. Для каждого — `SSH_HOST`,
  `SSH_USER`. (Один сервер на dev+prod — только для теста: универсальный compose не
  разводит порты, на одном хосте стенды подерутся.)
...
**Конфиг приложения**
- Какие переменные/секреты нужны приложению на каждом стенде (наполнение `APP_DOTENV`
  и Environment Secrets).
- Язык/тулчейн проекта (по умолчанию Python: ruff/pytest/pip-audit; иначе заменить шаги
  в `feature.yml`/`pr.yml`).
