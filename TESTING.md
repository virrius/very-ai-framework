...
**Ручной запуск:** Actions → `Main CI` → Run workflow (на любой ветке). В списке `markers`
выбираешь: `all` (по умолчанию — все, включая `heavy`), `heavy` (только тяжёлые) или
`not heavy` (без тяжёлых).
