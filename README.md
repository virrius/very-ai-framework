...
| `docker-compose.yml` | пример compose — перепиши под свои сервисы |
...
Деплой собирает `app.env` из этих значений и прокидывает его в контейнеры через `env_file`.
...
- если скилл установлен (см. ниже) — просто вызови `/setup-cicd`;
